...
int binary_tree_is_heap(const binary_tree_t *tree)
{
	...
	if (!binary_tree_is_complete(tree))
		return (0);
	if (!check_parent(tree))
		return (0);
	return (1);
}

/**
 * check_parent - checks if all nodes are greater than or equal to their children
 * @node: a pointer to the node to check
 * Return: 1 if all nodes are greater than or equal to their children, and 0 otherwise
 */
